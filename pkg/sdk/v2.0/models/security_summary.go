...
func (m *SecuritySummary) validateDangerousArtifacts(formats strfmt.Registry) error {
	if typeutils.IsZero(m.DangerousArtifacts) { // not required
		return nil
	}

	for i := 0; i < len(m.DangerousArtifacts); i++ {
		if typeutils.IsZero(m.DangerousArtifacts[i]) { // not required
			continue
		}
...
func (m *SecuritySummary) validateDangerousCves(formats strfmt.Registry) error {
	if typeutils.IsZero(m.DangerousCves) { // not required
		return nil
	}

	for i := 0; i < len(m.DangerousCves); i++ {
		if typeutils.IsZero(m.DangerousCves[i]) { // not required
			continue
		}
